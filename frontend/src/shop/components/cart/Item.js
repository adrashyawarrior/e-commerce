...
const Item = ({ id, index, image, name, price, quantity }) => {
    ...
    return (
        <div className="grid grid-cols-12 justify-between px-4 py-2 leading-normal items-center bg-gray-100 rounded-lg border shadow-md w-full mx-2 my-2">
            ...
            <div className="col-span-2">
                <button onClick={() => { removeProductHandler() }} type="button" className="text-red-400 hover:text-red-600 font-medium rounded-lg text-sm"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M18 12H6" />
                </svg>
                </button>
                <span className="px-4">{quantity || 'NA'}</span>
                <button onClick={() => { addProductHandler() }} type="button" className="text-green-400 hover:text-green-600 font-medium rounded-lg text-sm"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
            ...
            <div className="">
                <Link to="#" onClick={() => { removeProductHandler(true) }}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 text-red-300 hover:text-red-500">
